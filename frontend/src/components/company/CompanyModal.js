...
export default function CompanyModal({ mode, company, onClose, onSuccess, BACKEND_URL }) {
  ...
  const [form, setForm] = useState(
    isEdit
      ...
      : { companyName: "", brandName: "", GSTIN: "", companyAddress: "", clients: [] }
  );
  ...
  const addClient = () => {
    const { name, contactNumber } = clientTmp;
  ...
  const submit = async () => {
    if (!form.companyName) return setErr("Company name required");
    try {
      setSaving(true);
      const token = localStorage.getItem("token");
      if (isEdit) {
        await axios.put(`${BACKEND_URL}/api/admin/companies/${company._id}`, form, {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 z-50 flex items-center justify-center">
      <div className="bg-white rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6">
        <h2 className="text-2xl font-bold mb-4">{isEdit ? "Edit Company" : "Add Company"}</h2>
