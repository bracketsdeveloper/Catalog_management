...
export default function AddExpenseModal({ expense, onClose }) {
  ...
  // Filter ORDER_SECTIONS for non-super admins when orderConfirmed is true
  const filteredOrderSections = orderConfirmed && !isSuperAdmin
    ? ORDER_SECTIONS.filter(section => section !== "Product Cost" && section !== "Branding Cost")
    : ORDER_SECTIONS;
