...
export const excelUtils = {
  generateAttendanceTemplate() {
    ...
    const instructions = [
      ...
      [
        "Status",
        "Attendance status",
        "One of: Absent, WeeklyOff, Present, Absent (No OutPunch), ½Present, WeeklyOff Present",
        "Required",
      ],
  ...
  validateAttendanceData(data, requiredFields = ["E. Code"]) {
    ...
    data.forEach((row, index) => {
      ...
      if (row["E. Code"]) {
        ...
        if (empId.length < 3) warnings.push(`Row ${rowNum}: Employee ID "${empId}" seems too short`);
      }
      ...
      ["InTime(00:00)", "OutTime(00:00)"].forEach((timeField) => {
        if (row[timeField] && row[timeField] !== "") {
          ...
          if (!/^\d{1,2}:\d{2}$/.test(time) && isNaN(parseFloat(time))) {
            warnings.push(
              `Row ${rowNum}: Invalid time format in "${timeField}". Use HH:mm or decimal hours`
            );
          }
        }
      });
      ...
      if (row[statusKey]) {
        ...
        if (!validStatuses.some((valid) => status.toLowerCase().includes(valid.toLowerCase()))) {
          warnings.push(
            `Row ${rowNum}: Unusual status "${status}". Expected one of: ${validStatuses.join(", ")}`
          );
        }
      }
    });
  ...
  exportSummaryToExcel(data, month, year) {
    ...
    const summaryStats = [
      ...
      [
        ...
        `${(
          data.reduce((sum, emp) => sum + emp.summary.attendanceRate, 0) / data.length
        ).toFixed(2)}%`,
      ],
      [
        "Total Working Hours",
        `${data.reduce((sum, emp) => sum + emp.summary.totalHours, 0).toFixed(2)}`,
      ],
  ...
  exportCalendarToExcel(calendarData, employee, month, year) {
    const headers = ["Date", "Day", "Status", "In Time", "Out Time", "Work Hours", "OT Hours", "Remarks", "Holiday", "Weekend"];
  ...
  exportSalaryToExcel(records, month, year) {
    ...
    const worksheet = XLSX.utils.json_to_sheet(excelData, { header: headers });
    worksheet["!cols"] = headers.map((h) => ({ wch: Math.max(h.length, 10) }));
...
// ✅ SINGLE HRMS OBJECT (no duplicates)
export const HRMS = {
  // ─────────────────────────────────────────────────────────────────────────
  // EMPLOYEE MANAGEMENT
  // ─────────────────────────────────────────────────────────────────────────
  ...
  updateEmployee(employeeId, payload) {
    return api.put(`/api/hrms/employees/${encodeURIComponent(employeeId)}`, payload);
  },
  deleteEmployee(employeeId) {
    return api.delete(`/api/hrms/employees/${encodeURIComponent(employeeId)}`);
  },
  getEmployee(employeeId) {
    return api.get(`/api/hrms/employees/${encodeURIComponent(employeeId)}`);
  },
  searchUsers(q) {
  ...
  getEmployeeAttendance(employeeId, params = {}) {
    return api.get(`/api/attendance/employee/${encodeURIComponent(employeeId)}`, { params });
  },
  exportAttendance(params = {}) {
    return api.get("/api/attendance/export", { params, responseType: "blob" });
  },
  ...
  getEmployeeCalendar(employeeId, params = {}) {
    return api.get(`/api/attendance/employee/${encodeURIComponent(employeeId)}/calendar`, { params });
  },
  ...
  updateLeaveStatus(id, status, remarks = "") {
    return api.patch(`/api/hrms/leaves/${encodeURIComponent(id)}/status`, { status, remarks });
  },
  cancelLeave(id) {
    return api.post(`/api/hrms/leaves/${encodeURIComponent(id)}/cancel`);
  },
  getLeaveBalance(employeeId) {
    return api.get(`/api/hrms/leaves/balance/${encodeURIComponent(employeeId)}`);
  },
  ...
  updateHoliday(id, data) {
    return api.put(`/api/admin/holidays/${encodeURIComponent(id)}`, data);
  },
  deleteHoliday(id) {
    return api.delete(`/api/admin/holidays/${encodeURIComponent(id)}`);
  },
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // SALARY CONFIGURATION (✅ FIX: add upsertSalaryConfig)
  // ─────────────────────────────────────────────────────────────────────────

  // If your backend uses different paths, change ONLY these 3 lines.
  getSalaryConfig(employeeId) {
    const id = encodeURIComponent(employeeId);
    return api.get(`/api/configuration/salary-config/${id}`);
  },

  // ✅ This is what your UI was calling and failing
  upsertSalaryConfig(config) {
    return api.post("/api/configuration/salary-config", config);
  },

  // keep aliases for older code
  saveSalaryConfig(config) {
    return api.post("/api/configuration/salary-config", config);
  },

  getAllSalaryConfigs() {
    return api.get("/api/configuration/salary-configs");
  },
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // SALARY CONFIGURATION TEMPLATES
  // ─────────────────────────────────────────────────────────────────────────
  getSalaryConfigTemplates() {
    return api.get("/api/configuration/salary/config-templates");
  },
  applySalaryTemplate(data) {
    return api.post("/api/configuration/salary/apply-template", data);
  },
  ...
  calculateEmployeeSalary(employeeId, data) {
    return api.post(`/api/salary/calculate/${encodeURIComponent(employeeId)}`, data);
  },
  calculateAllSalaries(data) {
    return api.post("/api/salary/calculate-all", data);
  },
  previewSalary(employeeId, data) {
    return api.post(`/api/salary/preview/${encodeURIComponent(employeeId)}`, data);
  },
  ...
  getSalaryRecord(employeeId, month, year) {
    return api.get(`/api/salary/record/${encodeURIComponent(employeeId)}/${month}/${year}`);
  },
  updateSalaryRecord(id, data) {
    return api.put(`/api/salary/record/${encodeURIComponent(id)}`, data);
  },
  addSalaryAdjustment(id, adjustment) {
    return api.post(`/api/salary/record/${encodeURIComponent(id)}/adjustment`, adjustment);
  },
  approveSalaryRecord(id) {
    return api.post(`/api/salary/record/${encodeURIComponent(id)}/approve`);
  },
  markSalaryPaid(id, data = {}) {
    return api.post(`/api/salary/record/${encodeURIComponent(id)}/mark-paid`, data);
  },
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // CONFIG UTILS (kept from your file)
  // ─────────────────────────────────────────────────────────────────────────
  getDefaultCompanyConfig() {
    return {
      ...
      biWeeklyRule: { targetHours: 99, gracePeriodHours: 2, deductionPerHour: 500 },
      saturdaysPattern: "1st_3rd",
      leavePolicy: {
        sickLeave: { perMonth: 1, nonCumulative: true, cannotBeBundled: true },
        earnedLeave: {
        ...
        specialLeaves: { deathInFamily: 10, selfMarriage: 2 },
        holidays: { compulsoryPerYear: 10, restrictedPerYear: 2, canConvertToEL: true },
      ...
      wfhPolicy: {
        emergencyWFH: { salaryPercentage: 75, deductionPercentage: 25 },
        casualWFH: { salaryPercentage: 50, deductionPercentage: 50 },
        requiresPermission: true,
      },
      disciplinePolicy: {
        missedPunchPenalty: 250,
        ncncPenalty: { maxInstancesPerQuarter: 2, action: "termination" },
        overtimePolicy: { noMonetaryCompensation: true, forAppraisalOnly: true },
      },
      eligibilityPolicy: {
      ...
      incentives: {
        attendanceBonus: { amount: 1000, consecutiveMonths: 4, requires100Percent: true },
        // ✅ Accuracy Bonus (₹1000 monthly)
      ...
      statutoryDefaults: { pfPercentage: 12, esiPercentage: 0.75, esiSalaryThreshold: 21000, professionalTax: 200 },
      financialYear: { startMonth: 4, startDay: 1, endMonth: 3, endDay: 31 },
      version: "2026.1",
      policyDocumentPath: "",
    };
  },
  ...
  calculateWFHDeduction(
    ...
    casualWFHDays,
    dailyRate,
    emergencyDeductionRate = 0.25,
    casualDeductionRate = 0.5
  ) {
  ...
  exportConfigurationToExcel(configData, filename = "Company_Configuration.xlsx") {
    const headers = ["Setting Category", "Setting Name", "Value", "Description"];
    const excelData = [
      ...
      ["Incentives", "Accuracy Bonus Amount", configData.incentives?.accuracyBonus?.amount, "Accuracy bonus amount"],
      ["Incentives", "Accuracy Bonus Frequency", configData.incentives?.accuracyBonus?.frequency, "monthly/quarterly/yearly"],
    ...
    worksheet["!cols"] = [{ wch: 20 }, { wch: 25 }, { wch: 15 }, { wch: 40 }];
