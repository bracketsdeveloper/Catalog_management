...
const eInvoiceSchema = new mongoose.Schema(
  {
    // Use invoiceId for e-invoice lifecycle (unique+sparse prevents dup for null)
    invoiceId: {
    ...
    // (Optional: keep quotationId only if you still use it elsewhere; not unique)
    quotationId: {
      type: mongoose.Schema.Types.ObjectId,
      ref: "Quotation",
      index: true,
    ...
    // Buyer details (persisted for reference/build)
    customerDetails: {
      gstin: { type: String },
