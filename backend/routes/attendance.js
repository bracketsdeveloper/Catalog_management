...
function parseTimeValue(value) {
  ...
  // Handle numeric values (Excel stores times as fraction of day)
    ...
    typeof value === "number" ||
    (!isNaN(parseFloat(value)) && !String(value).includes(":"))
  ) {
...
// ─────────────────────────────────────────────────────────────────────────────
// ✅ SPECIAL LEAVE HELPERS (NEW)
// ─────────────────────────────────────────────────────────────────────────────

function normalizeSpecialLeaveType(v) {
...
async function isApprovedRestrictedHoliday(date, employeeId, userId) {
  ...
  const request = await RestrictedHolidayRequest.findOne({
    ...
    $or: [{ employeeId: employeeId }, { userId: userId }],
    status: "approved"
  }).lean();
...
async function findEmployeeWithFlexibleId(employeeIdFromExcel) {
  ...
  let employee = await Employee.findOne(
    ...
    { "personal.name": 1, "personal.employeeId": 1, "schedule": 1, "mappedUser": 1 }
  ).lean();
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        if (!employeeData) {
          ...
          if (employeeData) {
            ...
            results.warnings.push(
              `Row ${originalRowNum}: Employee not found for ID "${employeeIdFromExcel}"`
            );
            continue;
          }
        }
        ...
        // Calculate metrics if in/out present
        if (attendanceData.inTime && attendanceData.outTime) {
          ...
          attendanceData.totalDuration = `${totalHoursFloor.toString().padStart(2, "0")}:${totalMinutesRemainder
            .toString()
            .padStart(2, "0")}`;
          ...
          if (attendanceData.overTime) {
            ...
            attendanceData.hoursOT = attendanceData.hoursOT || 0;
          }

          const totalHours = attendanceData.hoursWorked + (attendanceData.hoursOT || 0);
          const totalHoursFloor = Math.floor(totalHours);
          const totalMinutesRemainder = Math.round((totalHours - totalHoursFloor) * 60);
          attendanceData.totalDuration = `${totalHoursFloor.toString().padStart(2, "0")}:${totalMinutesRemainder
            .toString()
...
router.get("/employee/:employeeId", authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    const [attendance, total] = await Promise.all([
      Attendance.find(query).sort({ date: -1 }).skip(skip).limit(parseInt(limit)).lean(),
    ...
    attendance.forEach((record) => {
      ...
      const isSpecial = !!record.specialLeaveType; // ✅ NEW

      if (isSpecial || status.includes("present")) {
        presentDays++;
  ...
  try {
    ...
    while (currentDate <= endDate) {
      ...
      if (leave) {
        ...
        if (attendanceRecord.specialLeaveType) {
          statusClass = "bg-emerald-100 text-emerald-800";
        } else if (statusLower.includes("absent") && leave) {
          status = `Leave (${leave.type})`;
          statusClass = "bg-blue-100 text-blue-800";
      ...
      calendarData.push({
        ...
        attendance: attendanceRecord
          ? {
              ...
              // ✅ NEW
              specialLeaveType: attendanceRecord.specialLeaveType || ""
            }
          : null,
        status,
        statusClass
    ...
    summary.attendanceRate =
      summary.workingDays > 0 ? ((summary.presentDays / summary.workingDays) * 100).toFixed(2) : 0;
    ...
    res.json({
      ...
      employee: {
        ...
        schedule: employee.schedule,
        leaveMonthlyAllocation: employee.leaveMonthlyAllocation || []
      },
...
router.get("/export", authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    const attendance = await Attendance.find(query).sort({ date: 1, employeeId: 1 }).lean();
    ...
    const excelData = attendance.map((record, index) => ({
      ...
      "Special Leave Type": record.specialLeaveType || "", // ✅ NEW (safe)
...
router.get("/summary", authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    const targetDate = date
      ...
      : parseDateString(new Date().toISOString().split("T")[0]);
    ...
    attendance.forEach((record) => {
      ...
      if (isSpecial || status.includes("present")) summary.present++;
...
// ─────────────────────────────────────────────────────────────────────────────
// MANUAL ENTRY ENDPOINT - UPDATED WITH SPECIAL LEAVE SUPPORT
// ─────────────────────────────────────────────────────────────────────────────

router.post("/manual", authenticate, requireAdmin, async (req, res) => {
  try {
    const { employeeId, date, inTime, outTime, status, remarks, specialLeaveType } = req.body;
    ...
    const { employee, matchedId } = employeeData;
    const special = normalizeSpecialLeaveType(specialLeaveType);
    ...
    // ✅ SPECIAL LEAVE OVERRIDE: Present + 9h slot
    if (special) {
      attendanceData.status = "Present";
      attendanceData.inTime = attendanceData.inTime || "10:00";
      attendanceData.outTime = attendanceData.outTime || "19:00";
      attendanceData.hoursWorked = 9;
      attendanceData.hoursOT = 0;
      attendanceData.workDuration = "09:00";
      attendanceData.overTime = "";
      attendanceData.totalDuration = "09:00";
      attendanceData.isLateArrival = false;
      attendanceData.isEarlyDeparture = false;
      attendanceData.lateByMinutes = 0;
      attendanceData.earlyByMinutes = 0;
      attendanceData.remarks = upsertSpecialLeaveTag(attendanceData.remarks, special);
    } else {
      // Normal calc
      if (inTime && outTime) {
        const metrics = calculateWorkMetrics(
    ...
    // Leave override (keep your behavior)
    const approvedLeave = await getApprovedLeaveForDate(matchedId, attendanceDate);
...
// ─────────────────────────────────────────────────────────────────────────────
// SELF SERVICE: MY MONTH SUMMARY (NO ADMIN REQUIRED)
// GET /attendance/me/summary?month=2&year=2026
// ─────────────────────────────────────────────────────────────────────────────
router.get("/me/summary", authenticate, async (req, res) => {
  try {
    ...
    const currentYear = year ? parseInt(year) : new Date().getFullYear();
    const currentMonthIdx = month ? parseInt(month) - 1 : new Date().getMonth();

    const startDate = new Date(Date.UTC(currentYear, currentMonthIdx, 1, 0, 0, 0, 0));
    const endDate = new Date(Date.UTC(currentYear, currentMonthIdx + 1, 0, 23, 59, 59, 999));

    const employee = await Employee.findOne(
      { mappedUser: req.user?._id, isActive: true },
      {
        "personal.employeeId": 1,
        "personal.name": 1,
        "org.role": 1,
    ...
    if (!employee?.personal?.employeeId) {
      return res
        .status(404)
        .json({ success: false, message: "Employee profile not linked to this user" });
    }

    const employeeId = employee.personal.employeeId;
    ...
    const expectedIn = parseTimeValue(employee.schedule?.expectedLoginTime);
    const expectedOut = parseTimeValue(employee.schedule?.expectedLogoutTime);
    ...
    attendance.forEach((record) => {
      ...
      if (isSpecial || status.includes("present")) {
        ...
        if (record.inTime && expectedIn) {
          const inParsed = parseTimeValue(record.inTime);
          if (inParsed && inParsed.totalMinutes > expectedIn.totalMinutes + 15) lateArrivals++;
        } else if (record.isLateArrival) lateArrivals++;

        if (record.outTime && expectedOut) {
          const outParsed = parseTimeValue(record.outTime);
          if (outParsed && outParsed.totalMinutes < expectedOut.totalMinutes - 15) earlyDepartures++;
        } else if (record.isEarlyDeparture) earlyDepartures++;
      } else if (status.includes("absent")) {
        absentDays++;
      } else if (status.includes("wfh")) {
        wfhDays++;
    ...
    const workingDays = getWorkingDaysInMonth(currentYear, currentMonthIdx) - publicHolidays.length;
    const expectedHours = workingDays * 8;
    ...
    res
      .status(500)
      .json({ success: false, message: "Error fetching my attendance summary", error: error.message });
  }
});
