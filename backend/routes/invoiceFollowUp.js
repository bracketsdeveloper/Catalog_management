...
router.get("/", authenticate, authorizeAdmin, async (req, res) => {
  try {
    ...
    deliveryReports.forEach((d) => {
      if (d.dispatchId) {
        deliveryReportMap[d.dispatchId.toString()] = d;
      }
    });

    // Merged entries from DispatchSchedule
    const merged = dispatchRows.map((d) => {
      const existing = followUpMap[d._id.toString()];
      const jobSheet = jobSheetMap[d.jobSheetNumber] || {};
      const delivery = deliveryReportMap[d._id.toString()] || {};
    ...
    // Include manually entered InvoiceFollowUp entries (no dispatchId)
    ...
    const allResults = [...merged, ...manualFollowUps];
